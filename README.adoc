= TechNexion Yocto 5.2 Walnascar 6.12.y GA BSP

NOTE: *This BSP is a TechNexion release providing support NXP i.mx series processors*

== Introduction
This article provides step-by-step instructions for building a Ubuntu image with TechNexion's Yocto BSP, which supports the i.MX series of processors.
...
- There are many repositories required. Google's `repo` tool is used to manage this.
[source,console]
$: mkdir ~/bin
...
[source,console]
$: repo init -u https://github.com/TechNexion/tn-imx-yocto-manifest.git -b walnascar_6.12.y-stable -m imx-6.12.49-2.2.0.xml
...
`DISPLAY` is the display type. This option only works on i.mx6 (i.mx6 Solo/Dual Lite/Dual/Quad) SOMs and doesn’t work for i.mx6ul, i.mx6sx and i.mx7.
...
*NOTE*: +
1. *X11 and Framebuffer are supported only on i.MX6 and i.MX7. i.MX8 and later require XWayland.* +
...
|
|fsl-imx-fb
|Framebuffer (not supported for i.MX8)
...
|'y' +
'all'
|'*y*' option is dependent on the 'WIFI_MODULE' setting. +
...
IMPORTANT: *Do not add the argument 'WIFI_FIRMWARE=y' if you have not placed the firmware files in "sources/meta-tn-imx-bsp/recipes-kernel/linux-firmware/files". It would result in build failure.* +
...
=== For EDM-IMX95
- *Yocto Xwayland image with DDR 4GB*
[source,console]
$: WIFI_FIRMWARE=y DISTRO=fsl-imx-xwayland MACHINE=edm-imx95-4gb source tn-setup-release.sh -b build-xwayland-edm-imx95-4gb
$: bitbake imx-image-full

- *Yocto Xwayland image with DDR 8GB*
[source,console]
$: WIFI_FIRMWARE=y DISTRO=fsl-imx-xwayland MACHINE=edm-imx95-8gb source tn-setup-release.sh -b build-xwayland-edm-imx95-8gb
$: bitbake imx-image-full

- *Yocto Xwayland image with DDR 16GB*
[source,console]
$: WIFI_FIRMWARE=y DISTRO=fsl-imx-xwayland MACHINE=edm-imx95-16gb source tn-setup-release.sh -b build-xwayland-edm-imx95-16gb
$: bitbake imx-image-full
...
- *Yocto Xwayland image*
[source,console]
$: WIFI_FIRMWARE=y DISTRO=fsl-imx-xwayland MACHINE=pico-imx91 source tn-setup-release.sh -b build-xwayland-pico-imx91
$: bitbake imx-image-full
...
- If using a configuration other than imx-image-full, put the following in local.conf:
[source,console]
...
== Image Deployment
- When the build completes, the generated release image is under directory **<build_dir>/tmp/deploy/images/[MACHINE]** +
Ex. PICO-IMX8MM
[source,console]
